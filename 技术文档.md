...
### 11.7 2026-08-16 数据备份功能退役与子路径部署适配

- 删除 `app/api/backups/route.ts` 与 `app/api/backups/[id]/route.ts`，访问原接口返回 404。
- 从 `app/page.tsx`、`app/components/backend/BackendOperations.tsx` 和 `app/lib/api.ts` 移除数据备份菜单、视图、组件与 API Client。
- 从 `server/database.ts` 的角色权限种子及 `server/policy.ts` 的权限类型中移除 `backup.read`、`backup.write`。
- 清理仅由备份页面使用的样式，测试同步改为断言备份入口、客户端方法、权限和接口文件均不存在。
- 历史 `backup_runs` 表暂不执行破坏性删除，避免在未完成数据保留评审前清除已有记录；该表不再被当前应用读写。
- 新增 `app/lib/base-path.ts`，统一浏览器 API 地址、邀请登记地址、二维码地址和会话 Cookie 路径。
- `next.config.ts` 读取 `NEXT_PUBLIC_BASE_PATH`，当前预发布构建值为 `/market-application`；根路径本地开发时保持空值。
- `.openai/hosting.json` 的数据库绑定为 `DB`，文件存储绑定为 `FILES`。

### 4.3 2026-08-16 隔离预发布环境

| 项目 | 当前配置 |
|---|---|
| 访问地址 | `https://dev.openlinks.org.cn/market-application/` |
| 应用内部监听 | `127.0.0.1:18182` |
| 系统服务 | `market-application-staging.service` |
| 发布目录 | `/srv/market-application/staging/releases/20260816-2115` |
| 当前版本软链接 | `/srv/market-application/staging/current` |
| 持久化状态目录 | `/srv/market-application/staging/shared/platform-state` |
| 反向代理 | 在现有 Nginx 配置中新增 `/market-application/` 路径 |
| 数据库绑定 | 项目独享的持久化 D1 兼容数据库 `DB` |
| 文件存储绑定 | 项目独享的持久化 R2 兼容对象存储 `FILES` |

隔离规则：

1. 不占用现有公网 `8081` 服务端口，应用仅监听本机回环地址 `18182`。
2. 不替换域名现有 `/` 和 `/oa/` 路由，只新增项目专属子路径。
3. 不连接、不建表、不迁移服务器已有 MySQL 库；部署前后的既有数据库清单保持不变。
4. 数据库和文件状态只写入项目专属持久化目录，不与其他应用共享运行目录。
5. 发布采用版本目录和 `current` 软链接，后续版本可独立切换，不覆盖其他服务文件。
6. SSH 密码、数据库密码、令牌及其他密钥不得写入仓库、文档、服务文件或提交记录。

当前限制：服务器尚未提供独立的托管云数据库实例、连接地址和数据库账号，因此本次建立的是预发布服务器上的项目隔离持久化主存储，不是外部托管云 MySQL 主库。`FILES` 当前同样由服务器上的 Wrangler Worker 运行时提供 R2 兼容持久化存储；获得正式云数据库和云对象存储账号后，应新增预发布实例、执行迁移和数据校验，再切换绑定，仍不得复用或修改现有 dev 数据库。

### 4.4 预发布运行与检查命令

```bash
systemctl status market-application-staging.service
journalctl -u market-application-staging.service -n 100 --no-pager
nginx -t
curl -I https://dev.openlinks.org.cn/market-application/
curl -I http://127.0.0.1:18182/market-application/
```

服务运行生产构建后的 Worker，由 Wrangler 提供 D1/R2 绑定及持久化目录。不得直接在该 Linux 环境使用 `vinext start` 启动当前 Worker 产物。

### 9.5 2026-08-16 预发布部署问题归档

| 报错现象 | 根因 | 最终解决方案 |
|---|---|---|
| Linux 上直接执行 `vinext start` 出现 `ERR_UNSUPPORTED_ESM_URL_SCHEME`，提示无法加载 `cloudflare:workers` | Node.js 原生运行时不能直接解析 Cloudflare Worker 专用模块协议 | 使用 Wrangler 本地 Worker 运行时承载生产构建，并为 `DB`、`FILES` 配置独立持久化目录 |
| 首次插入 Nginx 配置后 `nginx -t` 失败 | 自动写入时 `$host` 等 Nginx 变量被命令环境提前展开 | 未执行错误配置重载；恢复保存的原配置，使用转义后的变量重新插入，待 `nginx -t` 通过后再 reload |

### 12.6 2026-08-16 验证结果

- `npm run typecheck`：通过。
- `npm run lint`：0 个错误，保留 9 项既有警告。
- `npm test`：8/8 通过。
- 使用 `NEXT_PUBLIC_BASE_PATH=/market-application` 的生产构建：通过。
- 预发布主页、登录接口和已认证账户接口：HTTP 200。
- 文件上传：HTTP 201，返回存储后端为 `r2`；文件删除：HTTP 200。
- 已删除的 `/api/backups`：HTTP 404。
- 原有公网 `8081` 服务：HTTP 200；预发布服务保持 active。
- 部署前后未修改服务器既有 MySQL 数据库及其他应用路由。
### 9.6 2026-08-16 预发布静态资源 404

- 报错现象：`https://dev.openlinks.org.cn/market-application/` 首页返回 HTTP 200，但页面引用的 7 个 `/market-application/assets/*` 样式和脚本资源返回 HTTP 404。
- 根因：生产构建在 HTML 中使用了 `/market-application` 子路径；Wrangler Worker 负责动态请求，而 Nginx 原转发规则未为该子路径提供静态资源目录映射，导致资源请求进入 Worker 后找不到对应文件。
- 最终解决方案：在现有 Nginx 站点配置中新增 `/market-application/assets/` 到 `/srv/market-application/staging/current/dist/client/assets/` 的只读映射，并为 `/market-application/favicon.svg` 增加单文件映射；保留原有 `/market-application/` API/页面代理规则。
- 变更前先保存 `dev-openlinks.codex-static-assets.bak`，执行 `nginx -t` 通过后使用 `nginx -s reload`；校验失败时恢复备份配置。
- 修复结果：7 个资源全部 HTTP 200；预发布首页 HTTP 200；`/market-application/api/backups` HTTP 404；既有公网 `8081` 服务 HTTP 200；Nginx 和 `market-application-staging.service` 均为 active。
